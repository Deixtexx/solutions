namespace task02
{
    ...
    public class StudentService
    {
        ...
        {
            return _students.Where(s => s.Grades.Sum() / s.Grades.Count() >= minAverageGrade);
        }
        ...
        {
            return GroupStudentsByFaculty()
                .Select(students => new KeyValuePair<string, int>
                    (students.First().Faculty, students.Sum(s => s.Grades.Sum() / s.Grades.Count()) / students.Count()))
                .MaxBy(p => p.Value).Key;
        }
    }
}
