...
namespace task02tests
{
    public class StudentServiceTests
    {
        ...
        public void GetStudentsWithMinAverageGrade_ReturnsCorrectStudents() 
        {
            ...
            Assert.Equal(2, result.Count);
            Assert.Contains(result, s => s.Name.Equals("Иван"));
            Assert.Contains(result, s => s.Name.Equals("Петр"));
        }

        [Fact]
        public void GetStudentsOrderedByName_ReturnsCorrectStudentsOrder()
        {
            var result = _service.GetStudentsOrderedByName().ToList();
            var names = result.Select(s => s.Name);
            Assert.Equal(names, new List<string> { "Анна", "Иван", "Петр" });
        }

        [Fact]
        public void GroupStudentsByFaculty_ReturnsCorrectStudentsGrouping()
        {
